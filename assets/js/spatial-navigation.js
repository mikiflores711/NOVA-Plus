...
const requested=localStorage.getItem('nova-navigation-mode')||'auto';document.documentElement.dataset.deviceClass=tvDetected()?'tv':(matchMedia('(pointer:coarse)').matches?'touch':'desktop');document.documentElement.dataset.navigationMode=requested;
window.NOVA_NAV={setMode,prepare,restore,move};
addEventListener('keydown',e=>{if(!REMOTE_KEYS.has(e.key))return;document.documentElement.classList.add('remote-navigation');if(isEditable(document.activeElement)&&!['Escape','GoBack','BrowserBack'].includes(e.key))return;if(e.key.startsWith('Arrow')){e.preventDefault();move(e.key);return}if(['GoBack','BrowserBack'].includes(e.key)||(e.key==='Backspace'&&!isEditable(document.activeElement))){e.preventDefault();history.length>1?history.back():location.assign(location.pathname.includes('/tv/')?'../index.html':'index.html');return}if(e.key==='Escape'){const close=[...document.querySelectorAll('.open [aria-label="Cerrar"],.open .dialog-close,.open .modal-close,.player-close')].find(visible);if(close){e.preventDefault();close.click()}}},true);
